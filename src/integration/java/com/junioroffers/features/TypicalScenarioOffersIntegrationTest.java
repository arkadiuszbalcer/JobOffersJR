...
public class TypicalScenarioOffersIntegrationTest extends BaseIntegrationTest implements SampleJobOffersResponse {
    ...
    public TypicalScenarioOffersIntegrationTest() throws Exception {
    }
    @Container
    ...
    @Test
    public void user_want_to_see_offers_but_have_to_be_logged_and_external_server_should_have_some_offers() throws Exception {
        // step 1: there are no offers in external HTTP server
        ...
        String jsonWithTwoOffersOffers = performGetForTwoOffersMvcResult.getResponse().getContentAsString();
        List<OfferResponseDto> twoOffers = objectMapper.readValue(jsonWithTwoOffersOffers, new TypeReference<>() {
        });
        assertThat(offers).hasSize(2);
        OfferResponseDto expectedFirstOffer = twoNewOffers.get(0);
        OfferResponseDto expectedSecodOffer = twoNewOffers.get(1);
        assertThat(twoOffers).containsExactlyInAnyOrder(
                new OfferResponseDto(expectedFirstOffer.id(), expectedFirstOffer.companyName(), expectedFirstOffer.offerUrl(), expectedFirstOffer.salary(),expectedFirstOffer.position()),
                new OfferResponseDto(expectedSecodOffer.id(), expectedSecodOffer.companyName(), expectedSecodOffer.offerUrl(), expectedSecodOffer.salary(),expectedSecodOffer.position())
        );
        ...
        //given
        String offerAddedToDataBase = expectedFirstOffer.id();
        //when
        ResultActions getOffersById = mockMvc.perform(get("/offers/" + offerAddedToDataBase)
                        .contentType(MediaType.APPLICATION_JSON_VALUE)
        );
        //then
        String singleOfferByOfferUrlJson = getOffersById.andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString();
        ...
        //given && when
        ResultActions performGetForForOffers = mockMvc.perform(get(offersUrl)
                .contentType(MediaType.APPLICATION_JSON_VALUE)
        );
        //then
        MvcResult performGetForFourOffersMvcResult = performGetForForOffers.andExpect(status().isOk()).andReturn();
        String jsonWithFourOffersOffers = performGetForTwoOffersMvcResult.getResponse().getContentAsString();
        List<OfferResponseDto> fourOffers = objectMapper.readValue(jsonWithFourOffersOffers, new TypeReference<>() {
        });
        ...
        assertThat(fourOffers).contains(
                new OfferResponseDto(expectedThirdOffer.id(), expectedThirdOffer.companyName(), expectedThirdOffer.offerUrl(), expectedThirdOffer.salary(),expectedThirdOffer.position()),
                new OfferResponseDto(expectedFourthOffer.id(), expectedFourthOffer.companyName(), expectedFourthOffer.offerUrl(), expectedFourthOffer.salary(),expectedFourthOffer.position())
        );
